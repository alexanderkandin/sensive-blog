...
def serialize_post_optimized(post):
    return {
        ...
        'slug': post.slug,
        'tags': [serialize_tag(tag) for tag in post.tags.annotate(total_tags=Count('posts')).order_by('-total_tags')],
...
def serialize_tag(tag):
    return {
        'title': tag.title,
        'posts_with_tag': tag.total_tags,
    }


def index(request):
    most_popular_posts  = Post.objects.popular().prefetch_related('tags').select_related('author')[:5].fetch_with_comments()


    most_fresh_posts = Post.objects.annotate(
        total_likes=Count("likes",distinct=True),
    ).select_related('author').prefetch_related('tags').order_by('-published_at')[:5]
...
def post_detail(request, slug):
    ...
    related_tags = Tag.objects.popular()[:5]
